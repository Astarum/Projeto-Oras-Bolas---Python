  ...
  #apaga todos campos de digitação
    ...
    if liberado == True:
      ...
      for i in range(len(tempoFloat)):
        destinoX = bolaX[i] + funcoes.const_raio*math.cos(math.radians(53.1301023541))
        destinoY = bolaY[i] + funcoes.const_raio*math.sin(math.radians(53.1301023541))

        
        #print(aceleracaoRobo)
        #calcula o angulo
        funcoes.angulo(ang,destinoX,destinoY,roboX,roboY,i)  
        #corrige se necessario
        funcoes.quadrante(destinoX,destinoY,roboX[i],roboY[i],i,ang)
        ...
        if (raioY[0] > 0 and aux2Y >= destinoY - raioY[i]):
          aux2Y = destinoY
        ...
        elif (raioY[0] < 0 and aux2Y <= destinoY + math.fabs(raioY[i])):
          aux2Y = destinoY 
        ...
        if (raioX[0] > 0 and aux2X >= destinoX - raioX[i]):
          aux2X = destinoX 
        ...
        elif (raioX[0] < 0 and aux2X <= destinoX + math.fabs(raioX[i])):
          aux2X = destinoX
        ...
        tempo_freio = (velTotal[i+1]-funcoes.vel_alvo)/1.5
        indice = (math.fabs(tempo_freio))/0.02 
        indice_arredondado = int(indice)

        estimativaX = funcoes.deslocamento_RoboX(deslocamentoX,(aX[i]),velX[i],i,tempo_freio) 
        estimativaY = funcoes.deslocamento_RoboY(deslocamentoY,(aY[i]),velY[i],i,tempo_freio)
        # estimativaX*=2
        # estimativaY*=2
        dist_inst_tempo = math.sqrt((math.pow((bolaX[i] - estimativaX),2) + math.pow((bolaY[i] - estimativaY),2))) 
  
        
        if (xI<yI):
          calculus = (funcoes.const_raio+(funcoes.const_raio*0.1))*0.3
        else:  
          calculus = (funcoes.const_raio+(funcoes.const_raio*0.1))*0.5
        #print(f"Distancia = {distancia[i]} distancia necessario = {dist_inst_tempo} freio = {tempo_freio}   velTotal = {velTotal[i+1]} indice = {indice_arredondado} calculus = {calculus} ")
        if dist_inst_tempo<=calculus:
        ...
        if velTotal[i+1]<= funcoes.vel_alvo:
          velTotal[i+1] = funcoes.vel_alvo
          velX[i+1] = velTotal[i+1]*math.cos(math.radians(ang[i]))
          velY[i+1] = velTotal[i+1]*math.sin(math.radians(ang[i]))
...
def javascript():
  global frearX,velTotal,velX,velY,graficoTempo,grafico_bolaX,grafico_bolaY,roboX,roboY,distancia,aX, aY,grafico_aceleracao_BolaX, grafico_aceleracao_BolaY,grafico_velocidade_BolaX,grafico_velocidade_BolaY,bolaX,bolaY,tempoFloat,grafico_aceleracao_BolaY,grafico_aceleracao_BolaY,ultimo_elemento,deslocamentoX,deslocamentoY,ang,raioX,raioY,existem_dados,primeira_interacao_anterior,primeira_interacao_proximo,existem_dados,interacao_entre_botoes
  ...
  while fim == False:
    ...
    auX = bolaX[i]-novo_roboX[j+1]
    auY = bolaY[i]-novo_roboY[j+1]
    distc = math.sqrt((math.pow(auX,2) + math.pow(auY,2)))
    #print(distc)
  ...
  while chegou == False:
    ...
    # tempo_freio = (nova_velTotal[j+1]-funcoes.vel_alvo)/1.5
    # indice = (math.fabs(tempo_freio))/0.02
    # indice_arredondado = int(indice)
    ...
    #print(f"Distancia = {distancia[i]} distancia necessario = {dist_inst_tempo} freio = {tempo_freio}   velTotal = {velTotal[i+1]} indice = {indice_arredondado} calculus = {calculus} ")
    #if dist_inst_tempo<=calculus:
    #if dist_inst_tempo<=(calculus) and frear==False:
    #   frearX = True
    ...
    #   nova_vel_X[j+1] = nova_velTotal[j+1]*math.cos(math.radians(ang[i]))
    #   nova_vel_Y[j+1] = nova_velTotal[j+1]*math.sin(math.radians(ang[i]))
  ...
  print(len(aX_movimentando_bola))
  print(j)
  preparar_chute = False
  #print(novo_roboX)
  while preparar_chute == False:
    funcoes.velocidade_aceleracao_Robo(nova_velTotal,nova_vel_X,nova_vel_Y,aX_movimentando_bola,aY_movimentando_bola,aceleracaoRobo,novo_angulo[-1]+180,j,0.02)
    aux2X = funcoes.deslocamento_RoboX(novo_roboX,aX_movimentando_bola[j],nova_vel_X[j],j,0.02)
    if aux2X >= 4:
      aux2X = 4
     
    novo_roboX.insert(j+1,aux2X)  
    novo_roboY.insert(j+1,novo_roboY[-1])
    pos_bolaX.append(pos_bolaX[-1])
    pos_bolaY.append(pos_bolaY[-1])
    novo_angulo.append(novo_angulo[-1])
  ...
  while chute == False:  
    funcoes.velocidade_aceleracao_Robo(nova_velTotal,nova_vel_X,nova_vel_Y,aX_movimentando_bola,aY_movimentando_bola,aceleracaoRobo,novo_angulo[-1],j,0.02)
    aux2X = funcoes.deslocamento_RoboX(novo_roboX,aX_movimentando_bola[j],nova_vel_X[j],j,0.02)
    if aux2X <= 3-funcoes.const_raio* math.cos(auxiliar):
      aux2X = 3-funcoes.const_raio* math.cos(auxiliar)
     
    auxiliar = novo_angulo[-1]  
    novo_roboX.insert(j+1,aux2X)  
    novo_roboY.insert(j+1,novo_roboY[-1])
    pos_bolaX.append(pos_bolaX[-1])
    pos_bolaY.append(pos_bolaY[-1])
  ...
  randomizarTempo = [0.06,0.10,0.4,0.9,1.5,2.5]  
  tempo = randint(0,5)
  calc = randomizarTempo[tempo]/0.02
  velocidade_necessariaX = (pos_bolaX[-1]-0)/randomizarTempo[tempo]
  velocidade_necessariaY = (pos_bolaY[-1]-3)/randomizarTempo[tempo]
  velocidade_necessariaTotal = math.sqrt(math.pow(velocidade_necessariaX,2)+math.pow(velocidade_necessariaY,2))
  conta = 0
  while True:
    angulo_gol = funcoes.angulo_referencia(0,3,pos_bolaX[j],pos_bolaY[j])
    auxiliar = math.radians(angulo_gol)
    prox_pos_bolaX = pos_bolaX[j]+velocidade_necessariaTotal*math.cos(auxiliar)*0.02
    prox_pos_bolaY = pos_bolaY[j]+velocidade_necessariaTotal*math.sin(auxiliar)*0.02
    
    if prox_pos_bolaX<=0:
      prox_pos_bolaX = 0
    if prox_pos_bolaY<=3:
      prox_pos_bolaY = 3  
    #print(prox_pos_bolaX)  
    pos_bolaX.insert(j+1,prox_pos_bolaX)
    pos_bolaY.insert(j+1,prox_pos_bolaY)
    novo_roboX.append(novo_roboX[-1])  
    novo_roboY.append(novo_roboY[-1])
    novo_angulo.append(novo_angulo[-1])
